...
class EntityExtractionAgent:
    ...
    def _identify_speaker_type(self, speaker: str, text: str) -> str:
        ...
        # Default classification based on conversation flow
        return 'unknown'
    ...
    def _extract_conversational_entities(self, conversation_data: Dict) -> Dict:
        ...
        # Also check if any price numbers were mentioned
        if self.price_pattern.search(full_text) or 'rate' in full_text:
            entities['was_price_discussed'] = True
        ...
        # Check if number was exchanged
        if ('mobile number' in full_text or 'phone number' in full_text or 
            'number' in full_text or re.search(r'\d{10}', full_text) or
            self.fragmented_phone_pattern.search(full_text)):
    ...
    def _build_entities_object(self, deterministic: Dict, conversational: Dict) -> ExtractedEntities:
        ...
        return ExtractedEntities(
            ...
            # Store additional extracted data in special_requirements for now
            # (You may want to extend the ExtractedEntities model to include these)
